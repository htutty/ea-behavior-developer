...
namespace BehaviorDevelop
{
	...
	public partial class ElementForm : Form
	{
		...
		public ElementForm(ref ElementVO element) {
			...
			if( ProjectSetting.getVO().eaRepo == null ) {
		...
		private void setElementItems( ElementVO elem ) {
			...
			switch(elem.eaType) {
				case "Screen":
					elementTypePictureBox.Image = global::BehaviorDevelop.Properties.Resources.ICON_SYMBOL_SCREEN;
					break;
				case "GUIElement":
					elementTypePictureBox.Image = global::BehaviorDevelop.Properties.Resources.ICON_SYMBOL_UIELEMENT;
					break;
				default:
					elementTypePictureBox.Image = global::BehaviorDevelop.Properties.Resources.ICON_SYMBOL_CLASS;
					break;
			}
		...
		private void makeTaggedValueItems( List<TaggedValueVO> tagvals ) {
			...
			foreach( TaggedValueVO tv in tagvals ) {
				...
				tvSymbol.Image = global::BehaviorDevelop.Properties.Resources.ICON_SYMBOL_TAGGEDVALUE;
		...
		private void makeAttributeItems( List<AttributeVO> attrs ) {
			...
			foreach( AttributeVO a in attrs ) {
				...
				attrSymbol.Image = global::BehaviorDevelop.Properties.Resources.ICON_SYMBOL_ATTRIBUTE;
		...
		private void makeMethodItems( List<MethodVO> mths ) {
			...
			foreach( MethodVO m in mths ) {
				...
				methSymbol.Image = global::BehaviorDevelop.Properties.Resources.ICON_SYMBOL_METHOD;
...
//			MessageBox.Show("method text box double clicked");
		...
		void BtnCommitClick(object sender, EventArgs e)
		{
			...
			try { 
				...
				swe = new StreamWriter(outputDir + @"\elements\" + myElement.guid.Substring(1,1) + @"\" + myElement.guid.Substring(2,1) + @"\" + myElement.guid.Substring(1,36) + "_changed.xml");
...
//			diffForm.Show(this);
		...
		/// <summary>
		/// GUIDで指定されたメソッドの詳細ファイルをWinMerge.exeで開く
		/// </summary>
        ...
        private void ButtonJumpEAElement_Click(object sender, EventArgs e)
        {
            if (ProjectSetting.getVO() != null)
            {
                ...
                if(repo == null)
                {
                    MessageBox.Show("EAに接続していないため、この操作は実行できません");
                } else {
                    EA.Element elem = repo.GetElementByGuid(this.myElement.guid);
