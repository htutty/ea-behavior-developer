...
//		TextBox selectedTextBox = null;
        ...
        /// <summary>
        /// 要素差分確認フォームのコンストラクタ（差分ツールが出力した成果物XMLを開いた時）
        /// </summary>
        /// <param name="mergedElement">差分ツールが出力したマージ済み要素</param>
        public DiffElementForm(ElementVO mergedElement)
        {
            //
            // The InitializeComponent() call is required for Windows Forms designer support.
            //
            ...
            // TODO: Add constructor code after the InitializeComponent() call.
            //
            myElement = mergedElement;

            // 
            this.Text = "クラス: " + mergedElement.name + " " + mergedElement.guid;

            setElementItems(mergedElement);
        }


        /// <summary>
        /// 要素内の画面項目をセット(EA上の同要素との比較モード時)
        /// </summary>
        ...
        /// <summary>
        /// 要素内の画面項目をセット(比較ツールの時)
        /// </summary>
        /// <param name="mergedElement"></param>
        private void setElementItems(ElementVO mergedElement)
        {
        ...
        void addItemAttributes(ElementVO mergedElem, ref int rowIndex)
        {
            ...
            foreach (AttributeVO attr in mergedElem.attributes)
            {
                leftAttr = attr;
                rightAttr = attr;
                leftText = "";
                ...
                {
                    case 'U':
                        if( attr.srcAttribute != null && attr.destAttribute != null )
                        {
                            getDisagreedAttributeDesc(attr.srcAttribute, attr.destAttribute, ref leftText, ref rightText);
                        }
                        break;
                    case 'C':
                        getMonoAttributeDesc(attr, ref rightText);
                        break;
                    case 'D':
                        getMonoAttributeDesc(attr, ref leftText);
                        break;
                    default:
                        break;
                }
        ...
        void addItemAttributes(ElementVO mergedElem, ElementVO leftElem, ElementVO rightElem, ref int rowIndex)
		{
			...
			foreach (AttributeVO attr in mergedElem.attributes) {
				...
				switch (attr.changed) {
					case 'U':
                        leftAttr = attr.srcAttribute;
                        if(leftAttr == null)
                        {
                            leftAttr = searchAttributeByGuid(leftElement.attributes, attr.guid);
                        }

                        rightAttr = attr.destAttribute;
                        if (leftAttr == null)
                        {
                            rightAttr = searchAttributeByGuid(rightElement.attributes, attr.guid);
                        }
        ...
        void addItemMethods(ElementVO mergedElem, ref int rowIndex)
        {
            ...
            foreach (MethodVO m in mergedElem.methods)
            {
                ...
                {
                    case 'U':
                        if(m.srcMethod != null && m.destMethod != null)
                        {
                            getDisagreedMethodDesc(m.srcMethod, m.destMethod, ref leftText, ref rightText);
                        }
...
//                        rightMth = searchMethodByGuid(rightElement.methods, m.guid);
                        getMonoMethodDesc(m, ref rightText);
                        break;
                    case 'D':
//                        leftMth = searchMethodByGuid(leftElement.methods, m.guid);
                        getMonoMethodDesc(m, ref leftText);
                        break;
                    default:
                        break;
                }
...
//	            	setListItems(listR, rightText);
		...
		private void getDisagreedAttributeDesc(AttributeVO leftAttr, AttributeVO rightAttr, ref string leftText, ref string rightText) {
			...
			leftText = leftAttr.getComparedString(rightAttr);
            rightText = rightAttr.getComparedString(leftAttr);
...
//			}
            ...
            leftText = leftMth.getComparedString(rightMth);
			rightText = rightMth.getComparedString(leftMth);
